...
func (m *DynamicManager) AddTorrentFromURL(url string, savePath string, seedTime int, language string, seriesID int64, release indexer.ParsedRelease, infoHash string) error {
	client, err := m.getPrimaryClient()
	if err != nil {
		return err
	}
	err = client.AddTorrentFromURL(url, savePath, seedTime, language, seriesID, release, infoHash)
	if err != nil {
		return err
	}
	...
	if err := m.QueueStore.Insert(queueItem); err != nil {
		log.Printf("[Manager Error] CRITICAL: Failed to insert torrent hash %s to DB: %v", infoHash, err)
		return err
	}
...
func (m *DynamicManager) AddTorrentFromMagnet(magnet string, savePath string, seedTime int, language string, seriesID int64, release indexer.ParsedRelease, infoHash string) error {
	client, err := m.getPrimaryClient()
	if err != nil {
		return err
	}

	err = client.AddTorrentFromMagnet(magnet, savePath, seedTime, language, seriesID, release, infoHash)
	if err != nil {
		return err
	}
