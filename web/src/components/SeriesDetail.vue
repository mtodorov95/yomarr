...
<template>
  <div class="w-full max-w-4xl animate-fade-in">
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        ...
        <p class="text-xs text-slate-400 mt-0.5">
          Storage Path: <code class="text-slate-300 bg-black/40 px-1.5 py-0.5 rounded font-mono">{{ series.path }}</code>
        </p>
      </div>
    </div>
    ...
    <div v-else-if="chapters && chapters.length > 0" class="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden shadow-2xl">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-slate-700 bg-slate-900/40 text-slate-400 text-xs uppercase font-black tracking-wider">
    ...
    <div v-else class="text-slate-400 text-center py-12 bg-slate-800/40 border border-slate-800 rounded-2xl border-dashed">
